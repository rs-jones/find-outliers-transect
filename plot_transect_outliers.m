%
% out = plot_transect_outliers(ages_ka,new_ages_ka,transect_type,save_plot)
% out = plot_transect_outliers(ages_ka,new_ages_ka,transect_type,save_plot,mask)
% out = plot_transect_outliers(ages_ka,new_ages_ka,transect_type,save_plot,mask,time_lim,pos_lim)
%
% Plots exposure ages as either a horizontal or vertical transect, 
% separately for each nuclide, highlighting outliers in grey (possible 
% outliers in dark grey).
%
...
%
% new_ages_ka is a required struct, as with ages_ka, but with outliers
% identified using find_outliers_regress.m or find_outliers_transect.m.
%
...
function out = plot_transect_outliers(ages_ka,new_ages_ka,transect_type,save_plot,mask,time_lim,pos_lim)

  % Check inputs
  if (nargin < 3 || nargin > 7)
      error('plot_transect_outliers has wrong number of inputs!');
  end
  ...
  for b = 1:numel(NN_ind)
      ...
      if N_ind == 1
          ...
          outliers = new_ages_ka.Be10_outliers;
          if isfield(new_ages_ka,'Be10_outliers_possible')
              poss_outliers = new_ages_ka.Be10_outliers_possible;
          end
      ...
      elseif N_ind == 2
          ...
          outliers = new_ages_ka.Al26_outliers;
          if isfield(new_ages_ka,'Al26_outliers_possible')
              poss_outliers = new_ages_ka.Al26_outliers_possible;
          end
      ...
      % Find outliers
      outlier_log = ismember(ages_errs(:,1),outliers);
      if exist('poss_outliers')
          poss_outlier_log = ismember(ages_errs(:,1),poss_outliers);
      end
      ...
      if (strcmp(transect_type,'vert'))
          h = errbar(ages_errs(~outlier_log,1),N_pos(~outlier_log),ages_errs(~outlier_log,2),'-','horiz');  set(h,'Color',col(1,:),'LineWidth',1.5);
          h = errbar(ages_errs(outlier_log,1),N_pos(outlier_log),ages_errs(outlier_log,2),'-','horiz');  set(h,'Color',l_grey,'LineWidth',1.5);
          if exist('poss_outliers')
              h = errbar(ages_errs(poss_outlier_log,1),N_pos(poss_outlier_log),ages_errs(poss_outlier_log,2),'-','horiz');  set(h,'Color',d_grey,'LineWidth',1.5);
          end
          plot(ages_errs(~outlier_log,1),N_pos(~outlier_log),'o','MarkerSize',6,'MarkerEdgeColor',col(1,:),'MarkerFaceColor',col(2,:),'LineWidth',1);
          plot(ages_errs(outlier_log,1),N_pos(outlier_log),'o','MarkerSize',6,'MarkerEdgeColor',l_grey,'MarkerFaceColor',l_grey,'LineWidth',1);
          if exist('poss_outliers')
              plot(ages_errs(poss_outlier_log,1),N_pos(poss_outlier_log),'o','MarkerSize',6,'MarkerEdgeColor',d_grey,'MarkerFaceColor',d_grey,'LineWidth',1);
          end
      ...
      elseif (strcmp(transect_type,'horiz'))
          ...
          h = errbar(N_pos(~outlier_log),ages_errs(~outlier_log,1),ages_errs(~outlier_log,2),'-');  set(h,'Color',col(1,:),'LineWidth',2);
          h = errbar(N_pos(outlier_log),ages_errs(outlier_log,1),ages_errs(outlier_log,2),'-','horiz');  set(h,'Color',l_grey,'LineWidth',1.5);
          if exist('poss_outliers')
              h = errbar(N_pos(poss_outlier_log),ages_errs(poss_outlier_log,1),ages_errs(poss_outlier_log,2),'-','horiz');  set(h,'Color',d_grey,'LineWidth',1.5);
          end
          plot(N_pos(~outlier_log),ages_errs(~outlier_log,1),'o','MarkerSize',6,'MarkerEdgeColor',col(1,:),'MarkerFaceColor',col(2,:),'LineWidth',1);
          plot(N_pos(outlier_log),ages_errs(outlier_log,1),'o','MarkerSize',6,'MarkerEdgeColor',l_grey,'MarkerFaceColor',l_grey,'LineWidth',1);
          if exist('poss_outliers')
              plot(N_pos(poss_outlier_log),ages_errs(poss_outlier_log,1),'o','MarkerSize',6,'MarkerEdgeColor',d_grey,'MarkerFaceColor',d_grey,'LineWidth',1);
          end
      ...
      % Save figure
      if save_plot == 1
          fig_name = strcat(ages_ka.ages_name,'_ExposureAge_transect_outliers_',N_name,'_',scaling);
